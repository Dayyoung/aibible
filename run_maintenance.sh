...
/usr/bin/python3 fix_youtube_titles.py >> batch_job.log 2>&1
...
if [ ! -f .playlists_deleted ]; then
    ...
    /usr/bin/python3 delete_all_playlists.py >> batch_job.log 2>&1
...
/usr/bin/python3 manage_playlists.py >> batch_job.log 2>&1

# 4. Manage Chapter-Level Playlists (1,189 Chapters)
echo "[4/4] Running Chapter-Level Playlist Management..." >> batch_job.log
/usr/bin/python3 create_chapter_playlists.py >> batch_job.log 2>&1
