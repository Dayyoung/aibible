...
const translations = {
  en: {
    ...
    'hero-title': 'USTAXBOOST — US Tax & Incorporation Advisory',
    ...
    'receipt-header': 'BIBLEFORAI - USTAXBOOST RECEIPT',
  ...
  ko: {
    ...
    'hero-title': 'USTAXBOOST — 미국 세무 & 법인설립 자문',
    ...
    'foot-copy': '&copy; 2026 BibleForAI. All rights reserved.',
    'receipt-header': 'BIBLEFORAI - USTAXBOOST 영수증',
    'receipt-date': '주문 날짜',
    'receipt-txid': '거래 ID',
...
function applyTranslations() {
  ...
  document.title = currentLang === 'ko'
    ? 'BibleForAI - USTAXBOOST | 미국 세무 & 법인설립 자문'
    : 'BibleForAI - USTAXBOOST | US Tax & Incorporation Advisory';
...
function renderOrders() {
  const orderLogs = JSON.parse(localStorage.getItem('ustaxboost_orders') || '[]');
...
function saveLocalOrder(details) {
  const logs = JSON.parse(localStorage.getItem('ustaxboost_orders') || '[]');
  const jurisdiction = document.getElementById('order-jurisdiction').value;
  ...
  logs.unshift(record);
  localStorage.setItem('ustaxboost_orders', JSON.stringify(logs));
  renderOrders();
