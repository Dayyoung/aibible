// GLOBALBANK app state
const SERVICE_KEY = 'globalbank';
...
const translations = {
  en: {
    ...
    'hero-title': 'GLOBALBANK — US LLC Formation & EIN',
    ...
    'benefits-title': 'Why GLOBALBANK works',
    ...
    'foot-note': 'GLOBALBANK helps international founders build a practical path into the US market.',
    'foot-contact': 'Contact support: snsherocom@gmail.com',
    'foot-copy': '&copy; 2026 BibleForAI GLOBALBANK. All rights reserved.',
    'receipt-header': 'BIBLEFORAI - GLOBALBANK RECEIPT',
  ...
  ko: {
    ...
    'btn-orders': '내 주문내역',
    'hero-badge': '미국 시장 진출',
    'hero-title': 'GLOBALBANK — 미국 LLC 설립 & EIN 발급',
    ...
    'benefits-title': 'GLOBALBANK가 효과적인 이유',
    ...
    'order-button': '패키지 주문',
    'foot-note': 'GLOBALBANK는 글로벌 창업자가 미국 시장으로 진입할 수 있는 실질적인 경로를 제공합니다.',
    'foot-contact': '문의: snsherocom@gmail.com',
    'foot-copy': '&copy; 2026 BibleForAI GLOBALBANK. All rights reserved.',
    'receipt-header': 'BIBLEFORAI - GLOBALBANK 영수증',
    'receipt-date': '주문 날짜',
...
function renderPackages() {
  const container = document.getElementById('globalbank-packages');
  if (!container) return;
  const isKo = currentLang === 'ko';
  const catalog = packageCatalog.globalbank;
  ...
  container.innerHTML = catalog.packages.map(pkg => {
    ...
    return `
      <article class="package-card ${pkg.featured ? 'featured' : ''}">
        ...
        <button class="btn-buy" onclick="openPurchaseModal('globalbank', '${pkg.id}')">
